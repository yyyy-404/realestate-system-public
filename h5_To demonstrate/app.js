...
function getProperties() { return loadJSON(KEY.properties, DEFAULT_PROPERTIES); }
function setProperties(v) { saveJSON(KEY.properties, v); }
function getFavorites() { return loadJSON(KEY.favorites, []); }
function setFavorites(v) { saveJSON(KEY.favorites, v); }
function getContracts() { return loadJSON(KEY.contracts, []); }
function setContracts(v) { saveJSON(KEY.contracts, v); }
function getDrafts() { return loadJSON(KEY.drafts, []); }
function setDrafts(v) { saveJSON(KEY.drafts, v); }
...
function renderProperties(listEl, onlyFavorites = false) {
  ...
  listEl.querySelectorAll("[data-contract]").forEach(btn => {
    btn.addEventListener("click", () => {
      const id = Number(btn.dataset.contract);
...
function renderContracts(listEl) {
  ...
  listEl.querySelectorAll("[data-confirm]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const next = getContracts().map(c => c.id === id ? { ...c, status: "已完成" } : c);
      setContracts(next);
      toast("合同状态已更新");
...
function setupAuthRegister() {
  ...
  roleItems.forEach(item => {
    item.addEventListener("click", () => {
      ...
      form.role.value = item.dataset.role;
    });
  });
  form?.addEventListener("submit", (e) => {
    e.preventDefault();
    ...
    const users = loadJSON("rs_users", []);
    users.push({ username, password, role });
    saveJSON("rs_users", users);
    toast("注册成功");
...
function setupListPage() {
  ...
  const run = () => {
    ...
    if (kw) data = data.filter(i => [i.title, i.location, i.desc, i.owner].join(" ").includes(kw));
    if (st) data = data.filter(i => i.status === st);
    if (p) data = data.filter(i => Number(i.price) <= Number(p));
    const favs = new Set(getFavorites());
    ...
    list.querySelectorAll("[data-fav]").forEach(btn => btn.addEventListener("click", () => { addFavorite(Number(btn.dataset.fav)); run(); }));
    list.querySelectorAll("[data-contract]").forEach(btn => btn.addEventListener("click", () => {
      const property = getProperties().find(p => p.id === Number(btn.dataset.contract));
      if (property) { createContract(property); location.href = "contracts.html"; }
    }));
  };
